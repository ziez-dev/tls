...
// ── Connection handler functions (passed as function pointers to core) ───────
...
pub fn handleTlsConnection(
    ...
    stream: net.Stream,
    router: *ziez.Router,
    conn_config: ziez.ConnConfig,
    logger: ziez.Logger,
    tls_runtime_ptr: *anyopaque,
) void {
    const tls_runtime: *TlsRuntime = @ptrCast(@alignCast(tls_runtime_ptr));
    ...
    var entropy: [server_entropy_len]u8 = undefined;
    ziez.platform.fillRandomBytes(&entropy);

    var tls_server = server.init(
    ...
    var http_server = std.http.Server.init(&tls_reader, &tls_writer);

    const sni_val = tls_server.sni_hostname orelse "unknown";
    ziez.processRequests(allocator, &http_server, router, conn_config, logger, true, sni_val, null);
}
...
pub fn runRedirectListenerFn(
    ...
    const redirect_http: *const RedirectHttpConfig = @ptrCast(@alignCast(redirect_config_ptr));
    ...
    var srv = ip_addr.listen(io, .{}) catch |e| {
        logger.errorFields(.{
            .component = "listener",
    ...
    const shouldRedirectFn = struct {
        fn f(ptr: *anyopaque, path: []const u8) bool {
            return (@as(*const RedirectHttpConfig, @ptrCast(@alignCast(ptr)))).shouldRedirect(path);
        }
    }.f;
    ...
    while (true) {
        ...
        _ = std.Thread.spawn(.{}, ziez.handleRedirectConnection, .{
            allocator,
            stream,
            router,
            conn_config,
